...
| 消息模板 | [消息模板](../配置与审计/消息模板.md) | 模板消息、接收人、站内消息；实时推送见 SSE 专题 |
...
| 字典缓存 | [数据字典](../配置与审计/数据字典实现逻辑.md) | Caffeine + Redis 二级缓存架构 |
...
| 权限注解 `@RequirePerm` | [认证授权](../身份与权限/认证授权实现逻辑.md) | 按钮级权限校验注解 |
| 自动填充注解 | [后端文档导航](../README.md) | TenantMetaObjectHandler 自动填充（见手册能力总览表） |
| 字段加密注解 `@FieldEncrypt` | [加密功能实现逻辑](../模块专题/加密功能实现逻辑.md) | 字段透明加密/解密 |
