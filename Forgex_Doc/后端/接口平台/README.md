...
接口平台源码模块为 `Forgex_MOM/Forgex_Backend/forgex-admin/forgex-admin-integration/Forgex_Integration`，负责把内部业务数据转换为第三方请求，也负责接收第三方推送并转换为内部处理器参数。文档按完整功能组织；配置、授权、参数映射、调用、日志和内部集成分别有独立文件，但一次联调仍应按本文的顺序完成。
