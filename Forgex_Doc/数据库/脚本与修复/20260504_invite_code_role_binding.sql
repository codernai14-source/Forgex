...
-- ============================================================
-- 2026-05-04 邀请码绑定角色升级脚本
-- 适用库：forgex_admin、forgex_common
-- 可重复执行。
-- 修复内容：
-- 1. sys_invite_code 增加 role_id，邀请码必须绑定注册后角色。
-- 2. sys_invite_register_record 增加 role_id，记录邀请注册实际绑定角色。
-- 3. InviteCodeTable 增加绑定角色列与角色查询条件。
-- ============================================================
...
CALL `forgex_admin`.`fx_add_column_if_missing`('forgex_admin', 'sys_invite_code', 'role_id',
  'ALTER TABLE `forgex_admin`.`sys_invite_code` ADD COLUMN `role_id` bigint NULL DEFAULT NULL COMMENT ''注册后绑定角色ID'' AFTER `position_id`');
CALL `forgex_admin`.`fx_add_column_if_missing`('forgex_admin', 'sys_invite_register_record', 'role_id',
...
CALL `forgex_admin`.`fx_add_index_if_missing`('forgex_admin', 'sys_invite_code', 'idx_role_id',
  'ALTER TABLE `forgex_admin`.`sys_invite_code` ADD INDEX `idx_role_id` (`role_id`)');
CALL `forgex_admin`.`fx_add_index_if_missing`('forgex_admin', 'sys_invite_register_record', 'idx_role_id',
  'ALTER TABLE `forgex_admin`.`sys_invite_register_record` ADD INDEX `idx_role_id` (`role_id`)');

UPDATE `sys_invite_code` ic
JOIN `sys_role` r
