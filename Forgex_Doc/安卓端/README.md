...
| 能力 | 当前状态 |
|---|---|
| 工程结构 | 已支持 `app`、`core/*`（13 个）、`feature/*`（13 个）模块化拆分 |
...
| `core/database` | Room 本地数据库、缓存实体及离线队列持久化 |
...
| `core/sync` | WorkManager 离线同步及重试调度 |
...
| `feature/basic` | 基础数据移动端骨架；当前为占位页面，业务列表未完成 |
| `feature/equipment` | 设备资产功能骨架；当前为占位页面，点检/工单未完成 |
| `feature/home` | 首页导航、快捷入口与工作台聚合 |
| `feature/integration` | 集成平台移动端骨架；接口监控和任务状态未完成 |
| `feature/label` | 标签移动端骨架；打印和模板管理未完成 |
| `feature/message` | 消息中心、通知列表与已读回执 |
| `feature/production` | 生产工单移动端骨架；报工与进度查看未完成 |
| `feature/profile` | 个人中心、个人资料与设置 |
| `feature/quality` | 质量检验移动端骨架；不合格品处理未完成 |
| `feature/report` | 报表移动端骨架；看板查看未完成 |
| `feature/warehouse` | 仓储移动端骨架；出入库与盘点未完成 |
...
- 认证与租户逻辑：参考 [认证授权](../后端/身份与权限/认证授权实现逻辑.md)
- 网关与统一入口：参考 [网关与路由实现逻辑](../后端/模块专题/网关与路由实现逻辑.md)
- 工作流能力：参考 [工作流实现逻辑](../后端/模块专题/工作流实现逻辑.md)
...
已接入并可对照源码使用的能力是认证、租户选择、菜单拉取、会话持久化、网络统一结果、导航和工作流待办。`feature/basic`、`equipment`、`integration`、`label`、`production`、`quality`、`report`、`warehouse` 目录仍包含 `*PlaceholderScreen.kt`，当前仅完成模块、导航和页面占位；下一步应先补 `core/network` API、Repository、ViewModel、真实 Compose 列表和对应测试，再把导航表中的“骨架”改为“已完成”。
