# forgex-business 企业业务服务扩展区

> 分类：后端 / 架构
> 版本：**V1.0.0**
> 更新时间：**2026-09-19**
> 适用范围：公司内部二开团队基于 Forgex 平台开发独立业务服务

本目录是**企业业务服务扩展区**：存放公司内部基于 Forgex 平台开发的业务微服务。这里的服务**不进入平台 Reactor**（根 `pom.xml` 只聚合 `forgex-common` 与 `forgex-admin`），拥有独立的 parent、端口、数据库与配置，可以脱离本仓库单独建仓开发。
...
| 平台实现（禁止） | `forgex-admin-runtime`、`Forgex_Sys`、`Forgex_Auth` 等服务 JAR | 业务服务不得依赖平台实现与平台库表 |

版本统一由 `forgex-common-bom` 管理：
...
1. 从公司私服获取 `forgex-common-*` 系列构件（或本地 `mvn -pl forgex-common -am install` 安装）。
...
样例展示了两种形态：默认只有业务 Controller + Starter；`platform-api` profile 演示按需引入 `Forgex_Sys_Api` 并用 Feign 直连平台（`FORGEX_SYS_URL` 可配）。
