...
public class OssStorageService implements FileStorageService {
     ...
     * 构造函数
     * 
     * @param configJson OSS配置JSON字符串
     * @throws IOException 配置解析异常
     */
    public OssStorageService(String configJson) throws IOException {
        ...
        if (!StringUtils.hasText(endpoint) || !StringUtils.hasText(accessKeyId) 
            ...
            throw new IllegalArgumentException("OSS配置不完整，缺少必要参数");
        }
    }
    ...
    public void download(String filePath, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            log.error("OSS文件下载失败: {}", e.getMessage(), e);
            throw new IOException("OSS文件下载失败: " + e.getMessage(), e);
        } finally {
            if (inputStream != null) {
                try {
                    inputStream.close();
