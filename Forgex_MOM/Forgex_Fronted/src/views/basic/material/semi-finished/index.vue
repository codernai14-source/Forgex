  ...
  <div class="material-page">
    <div class="page-header">
      <div>
        ...
        <h1>{{ t('basic.material.pages.semiFinished.title') }}</h1>
        <p>{{ t('basic.material.pages.semiFinished.description') }}</p>
      </div>
      <a-space>
        <a-button v-permission="'basic:material:add'" type="primary" @click="openCreate">
          <PlusOutlined /> {{ t('basic.material.pages.semiFinished.add') }}
        </a-button>
      </a-space>
    </div>

    <FxDynamicTable ref="tableRef" table-code="SemiFinishedMaterialTable" :request="handleRequest" :row-selection="rowSelection" row-key="id">
      <template #toolbar>
        <a-button
    ...
    <a-modal v-model:open="detailVisible" :title="t('basic.material.pages.semiFinished.detail')" width="800px" :footer="null">
...
const dialogTitle = computed(() => (isEdit.value ? t('basic.material.pages.semiFinished.edit') : t('basic.material.pages.semiFinished.add')))
...
function resetForm() {
  ...
  form.materialCode = ''
  form.materialName = ''
  form.materialType = 'SEMI_FINISHED'
...
function handleDelete(record: any) {
  Modal.confirm({
    ...
    content: t('basic.material.pages.semiFinished.confirmDelete', { name: record.materialName || '' }),
    onOk: async () => {
...
<style scoped lang="less" src="@/styles/views/basic/material/semi-finished/index.less"></style>
