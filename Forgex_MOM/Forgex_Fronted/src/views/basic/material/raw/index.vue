  ...
  <div class="material-page">
    <div class="page-header">
      <div>
        ...
        <h1>{{ t('basic.material.pages.raw.title') }}</h1>
        <p>{{ t('basic.material.pages.raw.description') }}</p>
      </div>
      <a-space>
        <a-button v-permission="'basic:material:add'" type="primary" @click="openCreate">
          <PlusOutlined /> {{ t('basic.material.pages.raw.add') }}
        </a-button>
      </a-space>
    </div>

    <FxDynamicTable ref="tableRef" table-code="RawMaterialTable" :request="handleRequest" :row-selection="rowSelection" row-key="id">
      <template #toolbar>
        <a-button
    ...
    <a-modal v-model:open="detailVisible" :title="t('basic.material.pages.raw.detail')" width="800px" :footer="null">
...
const dialogTitle = computed(() => (isEdit.value ? t('basic.material.pages.raw.edit') : t('basic.material.pages.raw.add')))
...
function handleDelete(record: any) {
  Modal.confirm({
    ...
    content: t('basic.material.pages.raw.confirmDelete', { name: record.materialName || '' }),
    onOk: async () => {
...
<style scoped lang="less" src="@/styles/views/basic/material/raw/index.less"></style>
