...
http.interceptors.request.use(cfg => {
    ...
    activeReq++;
    // 获取全局加载器实例
    const gl = window.__globalLoader;
...
http.interceptors.response.use(async (resp) => {
    ...
    if (needReload(code)) {
        ...
        // 检查是否可以恢复租户
        const canRecoverTenant = !cfg.__fxTenantRecovered && // 未尝试过恢复租户
            !!account && // 存在账号信息
            !!tenantId && // 存在租户ID
